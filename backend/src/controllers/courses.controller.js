...
exports.updateCourse = async (req, res, next) => {
  try {
    ...
    if (
      ...
      dataToUpdate.categoryId !== undefined
    ) {
    ...
    if (
      dataToUpdate.categoryId !== undefined &&
      dataToUpdate.categoryId !== course.categoryId
    )
...
exports.restoreCourse = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin") {
      return res.status(403).json({
        success: false,
        error: "Not authorized to restore this course. Admin only.",
      });
    }
    ...
    await logAdminAction({
      ...
      resource: "course",
      resourceId: course.id,
      details: `Restored course: ${course.title}`,
    });
