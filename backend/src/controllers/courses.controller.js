...
exports.getCourses = async (req, res, next) => {
  try {
    ...
    courses.forEach(c => {
      if (typeof c.outcomes === 'string') {
        try { c.outcomes = JSON.parse(c.outcomes); } catch (e) { c.outcomes = []; }
      }
    });
...
exports.getCourse = async (req, res, next) => {
  try {
    ...
    if (course && typeof course.outcomes === 'string') {
      try { course.outcomes = JSON.parse(course.outcomes); } catch (e) { course.outcomes = []; }
    }
...
exports.updateCourse = async (req, res, next) => {
  try {
    ...
    if (dataToUpdate.outcomes !== undefined) {
      dataToUpdate.outcomes = JSON.stringify(dataToUpdate.outcomes);
    }
    ...
    // Invalidate course cache
    await clearCache('cache:/api/courses');
    if (updated && typeof updated.outcomes === 'string') {
      try { updated.outcomes = JSON.parse(updated.outcomes); } catch (e) { updated.outcomes = []; }
    }
