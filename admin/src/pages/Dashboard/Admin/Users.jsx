...
const Users = () => {
  ...
  const handleDelete = async (user) => {
    ...
    try {
      await apiRequest(`/v1/admin/users/${user.id}`, {
        method: 'DELETE',
      });
  ...
  const handleToggleBlock = async (user) => {
    ...
    try {
      ...
      setUsers((current) =>
        current.map((entry) =>
          entry.id === user.id ? result.data : entry
        )
      );
    } catch (requestError) {
      setError(
        requestError.message || 'Unable to update user.'
      );
    } finally {
      setBusyUserId(null);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="relative w-full md:w-96 group">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <MdSearch
              className="text-gray-400 group-focus-within:text-accent-cyan transition-colors"
              size={20}
            />
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="py-4 px-6 font-semibold text-sm text-gray-300">
                  User
                </th>
                ...
                <th className="py-4 px-6 font-semibold text-sm text-gray-300 text-right">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                filteredUsers.map((user) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <span
                          className={`flex items-center gap-1.5 text-sm font-medium ${statusConfig.className}`}
                        >
                          <StatusIcon size={16} />
                          {statusConfig.label}
                        </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button
                        ...
                        disabled={isBusy}
                        className="p-2 rounded-lg text-red-400 hover:bg-red-400/10 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                        title="Delete User"
                      >
                        <MdDelete size={18} />
                      </button>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-medium border bg-white/5 text-gray-300 border-white/10">
                      {ROLE_LABELS[user.role] || user.role}
                    </span>
